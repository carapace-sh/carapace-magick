...
- ~~**Inline image generators**: `xc:red`, `gradient:`, `plasma:` — should these be completed as image inputs? They have their own sub-syntax. Low priority.~~ **Done** — `ActionInlineImages()` completes inline generator prefixes (`xc:`, `gradient:`, `plasma:`, etc.) and is wired into `ActionImageInput()`.
- ~~**Image format prefix**: `png:filename` — the `format:` prefix on filenames. Should completion handle this? Probably as a `ActionMultiParts(":")` like ffmpeg's stream specifier.~~ **Done** — `ActionFormatPrefixedFiles()` uses `ActionMultiParts(":")` to complete format prefixes then files, wired into `ActionImageInput()` and `ActionImageOutput()`.
