...
func bashBleSingle(command string) string {
	return fmt.Sprintf(`%v
...
_%[2]v_completion_ble() {
  if [[ ${BLE_ATTACHED-} ]]; then
    ...
    local IFS=$'\n'
    local c
    mapfile -t c < <(echo "$compline" | sed -e "s/ \$/ ''/" -e 's/"/\"/g' | xargs %[2]v %[3]v _carapace bash-ble)
    [[ "${c[*]}" == "" ]] && c=() # fix for mapfile creating a non-empty array from empty command output
    ...
    complete -F _%[3]v_completion %[3]v
  fi
}

complete -F _%[2]v_completion_ble %[3]v
`, bashSingle(command), executable(), command)
}
