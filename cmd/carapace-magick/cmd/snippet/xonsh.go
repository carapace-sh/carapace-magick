...
def _carapace_magick_completer(context):
    """carapace completer for ImageMagick"""
    ...
    output, _ = sub_proc_get_output(
        '%[1]v', context.command, '_carapace', 'xonsh', *[a.value for a in context.args], fix_prefix(context.prefix)
    )
...
add_one_completer('carapace-magick', _carapace_magick_completer, 'start')
`, executable(), strings.Join(complete, ", "))
}

func xonshSingle(command string) string {
	return fmt.Sprintf(`from xonsh.completers.completer import add_one_completer
from xonsh.completers.tools import contextual_command_completer

@contextual_command_completer
def _%[2]v_completion(context):
    """carapace completer for %[2]v"""
    if context.command != '%[2]v':
        return
    ...
    output, _ = sub_proc_get_output(
        '%[1]v', '%[2]v', '_carapace', 'xonsh', *[a.value for a in context.args], fix_prefix(context.prefix)
    )
...
add_one_completer('%[2]v', _%[2]v_completion, 'start')
`, executable(), command)
}
